...
class DB
{
    ...
    public function select($table, $fields = "*", $where = null, $orderBy = null, $direction = 'DESC')
    {
        if (is_array($fields))
        ...
        $orderByString = '';
        if (!empty($orderBy)) {
            $orderByString = "ORDER BY {$orderBy} {$direction}";
        }

        $sql = "SELECT {$fields_string} FROM {$table} {$where_string} {$orderByString}";
